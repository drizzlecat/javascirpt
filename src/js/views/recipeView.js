...
class recipeView extends Views{
 ...
 _errorMessage = 'We could not find the recipe, please try another one';
  ...
  addHandlerRender(handler){
    const arrays = ['hashchange','load'];
    arrays.forEach(element => window.addEventListener(element,handler));
  }
  ...
  addHandleUpdateServings(handler){
    ...
    this._parentElemnt.addEventListener('click',function(e){
      ...
      const dataUpdateTo = +btn.dataset.updateTo;
      // console.log(dataUpdateTo);
      handler(dataUpdateTo);
    })
  }
    ...
    return `
      ...
      <div class="recipe__ingredients">
        ...
        <ul class="recipe__ingredient-list">
         ${
            this._data.ingredients.map(element => {
            if(element.quantity)  return this.#generateIngredient(element);
          }).join('')
         }
        </ul>
      </div>
 ...
 #generateIngredient(element){
    return `
    <li class="recipe__ingredient">
      <svg class="recipe__icon">
        <use href="${icons}#icon-check"></use>
      </svg>
      <div class="recipe__quantity">${new Fraction(element.quantity).toString()}</div>
